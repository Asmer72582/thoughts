...
if (isset($_POST['submit'])) {
    ...
    if ($to_user == $username) {
       ...
       $query = "INSERT INTO `comment`(`comment`, `poet`, `post`, `c_time`, `poet_id`) 
        ...
        $re = mysqli_query($con, $query);
        if ($re==1) {
          $insert = "INSERT INTO `comment_notifications`(`from_user`, `to_user`, `comment`, `post_id`, `status`, `poet_id`, `id_post`)
          VALUES ('$from_user', '$to_user', '$comment', '$poem', '', '$idshow', '$poem_id')";
            ...
            $photo = "SELECT `id`, `title`, `poetid`, `textt`, `dateonly`, `foreginkey`, `bookmark`, `likes` FROM `writescd` WHERE id = $id ";
             ...
             <div class="col-lg-5 col-sm-12  my-4 " style="align-items: center;"  >
                  ...
                  <?php $title_poem = $result['title']; ?>
									...
									<!--zzzzzzzzzzzzzzzzzzzzzzzzzzz -->
			    					<h4 class="text-dark">by: <a class="text-warning" href="profileshow.php?id=<?php echo $result['poetid']; ?>"><?php echo $result['poetid'];
